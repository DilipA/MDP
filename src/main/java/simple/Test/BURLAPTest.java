 ...
 */
public class BURLAPTest extends MDPSolver implements Planner, QProvider {
    ...
    public BURLAPTest(SADomain domain, double gamma,
    ...
    public static void main(String [] args){
        ...
        gwd.setTf(new GridWorldTerminalFunction(1,1));

        //only go in intended directon 80% of the time
        gwd.setProbSucceedTransitionDynamics(1.0);
        ...
        BURLAPTest vi = new BURLAPTest(domain, 0.99, new SimpleHashableStateFactory(),
        ...
        for(int i=0;i < width;i++){
            for(int j=0;j < height;j++){
                State st = new GridWorldState(new GridAgent(i,j));
                System.out.println("Value(" + i + "," + j + ") = " + vi.value(st));
            }
        }
